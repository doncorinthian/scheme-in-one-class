...
import static sioc.Workarounds.asInstance;
...
class SIOC {
        ...
        private Object F_read() throws Throwable {
                return F_read(get("input"));
        }
        ...
        private void F_display(Object x) throws Throwable {
                F_display(x, get("output"));
        }
        ...
        private void F_print(Object x) throws IOException {
                F_print(x, get("output"));
        }
        ...
        private void F_newline() throws Throwable {
                F_newline(get("output"));
        }
        ...
        private static Object parse(Reader port, int[] nextc) throws IOException {
                ...
                for (; ; ) {
                        switch (c) {
                                ...
                                case ')':
                                        return K_HBend_of_list;
                                case EOF:
                                        return K_HBend_of_file;
                                case ';':
                                        for (; ; ) {
                                                c = port.read();
                                                switch (c) {
                                ...
                                case '"':
                                        return parseQuoted(port, '"');
                                case '|':
                                        return SF_string_Gsymbol(parseQuoted(port, '|'));
                                case '\'':
                                        return SF_list(S_quote, parse(port, nextc));
                                case '`':
                                        return SF_list(S_quasiquote, parse(port, nextc));
                                case ',':
                                        c = port.read();
                                        if (c == '@')
        ...
        private static Object computeMetaMapFunctions(Object[] map, String name) {
                ...
                switch (mhCount) {
                        case 0:
                                return NO_METHOD_HANDLES_ARRAY;
                        case 1:
                                return mhs.get(0);
                        default:
                                return mhs.toArray(new MethodHandle[mhCount]);
                }
        }
        ...
        // arithmetic
        private static int SF_P(int x, int y) {
                return x + y;
        }

        private static int SF__(int x, int y) {
                return x - y;
        }

        private static int SF__(int y) {
                return -y;
        }

        private static int SF_S(int x, int y) {
                return x * y;
        }

        private static int SF_D(int x, int y) {
                return x / y;
        }

        private static long SF_P(long x, long y) {
                return x + y;
        }

        private static long SF__(long x, long y) {
                return x - y;
        }

        private static long SF__(long y) {
                return -y;
        }

        private static long SF_S(long x, long y) {
                return x * y;
        }

        private static long SF_D(long x, long y) {
                return x / y;
        }

        private static double SF_P(double x, double y) {
                return x + y;
        }

        private static double SF__(double x, double y) {
                return x - y;
        }

        private static double SF__(double y) {
                return -y;
        }

        private static double SF_S(double x, double y) {
                return x * y;
        }

        private static double SF_D(double x, double y) {
                return x / y;
        }
